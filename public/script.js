...
function updateCartModalSummary() {
    ...
    // I prezzi sono già IVA inclusa (come dichiarato nei Termini di Servizio):
    // la riga IVA è solo una scomposizione informativa, NON va sommata al totale.
...
function updateCartSummary() {
    const total = state.cart.reduce((sum, item) => sum + item.price, 0);
    // I prezzi sono già IVA inclusa: la riga IVA è solo informativa.
    const vat = total - (total / 1.22);
